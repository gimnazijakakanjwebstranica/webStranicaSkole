...
import { Pomocno } from "../models/pomocnoModel.js";
...
router.post("/", async (req, res) => {
  try {
    ...
    const employee = await Pomocno.create(newEmployee);
...
router.get("/", async (req, res) => {
  try {
    const employee = await Pomocno.find({});
...
router.delete("/:id", async (req, res) => {
  const { id } = req.params;
  try {
    const result = await Pomocno.findByIdAndDelete(id);
