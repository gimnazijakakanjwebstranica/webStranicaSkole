...
const ListUprava = () => {
  ...
  const [upravaIdToDelete, setUpravaIdToDelete] = useState(null); // Store professor ID to delete

  const showModalToDelete = (id) => {
    setShowModal(true);
    setUpravaIdToDelete(id);
  };

  const hideModal = () => {
    setShowModal(false);
    setUpravaIdToDelete(null);
  };

  const confirmDelete = () => {
    if (upravaIdToDelete) {
      deleteUprava(upravaIdToDelete);
    }
  };

  const deleteUprava = async (id) => {
    setLoading(true);
    try {
      await axios.delete(`http://localhost:5555/uposlenici/uprava/${id}`);
      setLoading(false);
