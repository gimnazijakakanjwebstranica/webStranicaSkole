...
const ListAdministrativno = () => {
  ...
  const [administrativnoIdToDelete, setAdministrativnoIdToDelete] =
    useState(null); // Store professor ID to delete

  const showModalToDelete = (id) => {
    setShowModal(true);
    setAdministrativnoIdToDelete(id);
  };

  const hideModal = () => {
    setShowModal(false);
    setAdministrativnoIdToDelete(null);
  };

  const confirmDelete = () => {
    if (administrativnoIdToDelete) {
      deleteAdministrativno(administrativnoIdToDelete);
    }
  };

  const deleteAdministrativno = async (id) => {
    setLoading(true);
    try {
      await axios.delete(
...
export default ListAdministrativno;
