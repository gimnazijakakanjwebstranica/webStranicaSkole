...
const ListVijece = () => {
  ...
  const [vijeceIdToDelete, setVijeceIdToDelete] = useState(null);
  ...
  const showModalToDelete = (id) => {
    setShowModal(true);
    setVijeceIdToDelete(id);
  };

  const hideModal = () => {
    setShowModal(false);
    setVijeceIdToDelete(null);
  };

  const confirmDelete = () => {
    if (vijeceIdToDelete) {
      deleteVijece(vijeceIdToDelete);
    }
  };

  const deleteVijece = async (id) => {
    setLoading(true);
    try {
      await axios.delete(
        `http://localhost:5555/za-roditelje/vijece-roditelja/${id}`
      );
      setLoading(false);
